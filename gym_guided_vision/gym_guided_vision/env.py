...
from gym_guided_vision.constants import (
...
class GuidedVisionEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        left_gripper = action[6] # val from 0 to 1   (1 is open, 0 is closed)
        right_joints = action[7:13]
        right_gripper = action[13] # val from 0 to 1         (1 is open, 0 is closed)    
    ...
    # def old_obs_IGNORE(self) -> np.ndarray:
    #     left_qpos = self._physics.bind(self._left_joints).qpos.copy()
    #     left_qpos[6] = self.left_gripper_norm_fn(left_qpos[6])
    #     right_qpos = self._physics.bind(self._right_joints).qpos.copy()
    #     right_qpos[6] = self.right_gripper_norm_fn(right_qpos[6])
    ...
    #                     height=self.observation_height, 
    #                     width=self.observation_width, 
    #                     camera_id='zed_cam_left'
    #                 ),
    #                 self._physics.render(height=480, width=640, camera_id='zed_cam_right'),
    #             ], axis=1)
    ...
    # def step_OLD(self, action: np.ndarray) -> tuple:
...
if __name__ == '__main__':
    ...
    env = make_sim_env('sim_slot_insertion', cameras=[])
